...
Bodies do not penetrate each other and simulation is guided
by external forces like gravity, air drag and shooting.
...
\ref{tab:eom-g} and figure \ref{fig:eom-contact}.
...
\node[above] at (S) [align=center] {\large{$\sigma=f_y$}};
\end{tikzpicture}
\caption{Stress distribution for bending under elastic and plastic loads.}
\label{fig:wp}
\end{figure}
...
Maximum force acting in same direction as $\vec{r}_{anc}^{\,i} $
is product of area and yield stress as given in equation \ref{eq:fN}.
...
are product of area and shear yield stress $\tau_y$ as given in equation \ref{eq:fQ}.
...
and yield stress $f_y$ as given in equations \ref{eq:Mx} for moment around
x-axis and \ref{eq:Mz} for moment around z-axis.
...
Yield shear stress is assumed here to be $ 0.5\, f_y$ using Tresca yield critetion.
